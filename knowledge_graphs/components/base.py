...
class BaseComponent(ABC):
    ...
    def validate_outputs(self, state: PipelineState) -> List[str]:
        """
        Validate that the component produced expected outputs.

        Args:
            state: Pipeline state after component processing

        Returns:
            List of warning messages. Empty list means outputs are valid.
        """
        return []
...
class Scanner(BaseComponent):
    ...
    def validate_inputs(self, state: PipelineState) -> List[str]:
        errors = []
        if not state.get("input_path"):
            errors.append("Scanner requires 'input_path' in state")
        return errors

    def validate_outputs(self, state: PipelineState) -> List[str]:
        errors = []
        if not state.get("file_paths"):
            errors.append("Scanner produced no file paths")
        return errors
...
class Reader(BaseComponent):
    ...
    def validate_inputs(self, state: PipelineState) -> List[str]:
        errors = []
        if not state.get("file_paths"):
            errors.append("Reader requires 'file_paths' from Scanner")
        return errors

    def validate_outputs(self, state: PipelineState) -> List[str]:
        errors = []
        if not state.get("chunks"):
            errors.append("Reader produced no chunks")
        return errors
...
class Splitter(BaseComponent):
    ...
    def validate_inputs(self, state: PipelineState) -> List[str]:
        errors = []
        if not state.get("chunks"):
            errors.append("Splitter requires 'chunks' from Reader")
        return errors

    def validate_outputs(self, state: PipelineState) -> List[str]:
        errors = []
        if not state.get("split_chunks"):
            errors.append("Splitter produced no split chunks")
        return errors
...
class Extractor(BaseComponent):
    ...
    def validate_inputs(self, state: PipelineState) -> List[str]:
        errors = []
        if not state.get("split_chunks") and not state.get("chunks"):
            errors.append("Extractor requires 'split_chunks' or 'chunks'")
        return errors

    def validate_outputs(self, state: PipelineState) -> List[str]:
        errors = []
        if not state.get("subgraphs"):
            errors.append("Extractor produced no subgraphs")
        return errors
...
class Vectorizer(BaseComponent):
    ...
    def validate_inputs(self, state: PipelineState) -> List[str]:
        errors = []
        if not state.get("subgraphs"):
            errors.append("Vectorizer requires 'subgraphs' from Extractor")
        return errors

    def validate_outputs(self, state: PipelineState) -> List[str]:
        errors = []
        if not state.get("vectorized_subgraphs"):
            errors.append("Vectorizer produced no vectorized subgraphs")
        return errors
...
class Writer(BaseComponent):
    ...
    def validate_inputs(self, state: PipelineState) -> List[str]:
        errors = []
        if not state.get("vectorized_subgraphs") and not state.get("subgraphs"):
            errors.append("Writer requires 'vectorized_subgraphs' or 'subgraphs'")
        return errors

    def validate_outputs(self, state: PipelineState) -> List[str]:
        errors = []
        if not state.get("output_path"):
            errors.append("Writer produced no output path")
        return errors